...
быть, кнопками. Рассмотрим один из вариантов описания методов для вызова такого компонента:
...
нужной переменной — опыт работы с таким пакетом не изменится. Идея заключается в том, чтобы создать два похожих
