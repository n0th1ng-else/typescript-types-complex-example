...
работать не будет. Каждый объект из импорта модуля будет иметь тип `any`, что не сильно помогает в дальнейшем.
...
- Объявления типов будут выступать в качестве дополнительной документации к UI компонентам. Ведь выбирать метод из
  подсказок **intellisence** гораздо удобнее, чем идти на сайт с описанием нужного компонента и копировать имя метода.
